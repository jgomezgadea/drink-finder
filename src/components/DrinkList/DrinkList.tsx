...
const DrinkList = () => {
  ...
  const filteredDrinks = state.drinks
    ...
    // Filter by category
    .filter(drink => (filters.category.length === 0 ? true : filters.category.includes(drink.strCategory)))
    // Filter by glass
    .filter(drink => (filters.glass.length === 0 ? true : filters.glass.includes(drink.strGlass)));

  return (
    <div className="drinkList">
      {filteredDrinks.length ? (
        filteredDrinks.map(drink => <DrinkItem drink={drink} key={drink.idDrink} />)
      ) : state.drinks.length ? (
